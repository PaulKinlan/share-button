...
class ShareButton extends HTMLElement {
    ...
    this.android.addEventListener('click', e => {
      if(!!navigator.share) {
        navigator.share({ title: this.text, url: this.href })
                   .then(() => {
                     this.dispatchEvent(new Event('share-url', {bubbles: true, composed: true}));
                   });
      }
      else {
  ...
  mailUrl() {
    ...
    this.dispatchEvent(new Event('mail-url', {bubbles: true, composed: true}));
  }
  ...
  copyUrl() {
    ...
    try {
      ...
      window.getSelection().removeAllRanges();
      this.toggleOverlay();
      this.dispatchEvent(new Event('copy-url', {bubbles: true, composed: true}));
      return result;
    }
    catch(error) {
      console.error(error);
